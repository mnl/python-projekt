...
class Yatzy(ttk.Frame):
    ...
    def __init__(self, master=None):
        ...
        self.filemenu.add_command(
            label="Nytt spel", command=self.playAgain
        )  # Add menu option to entry
        self.filemenu.add_command(
            label="Rensa sparat spel", command=resetSave
        )  # Add menu option to entry
        self.filemenu.add_command(
            label="Avsluta", command=quit
        )  # Add menu option to entry
        self.helpmenu.add_command(label="Om programmet", command=about)

        # Add images
        self.banner_photo = PhotoImage(file="images/banner.png")
        ttk.Label(self.frame, image=self.banner_photo).grid(
        ...
        newBtn = ttk.Button(self.frame, text="Nytt Spel", command=self.newGame).grid(
            row=2, column=1
        )
        contBtn = ttk.Button(
            self.frame,
            text="Fortsätt Spela",
            command=self.loadAutoSave,
            state="disabled",
        )
        if os.path.isfile("saves/autosave.sav"):
            contBtn.state(
                ["!disabled"]
            )  # Enable button if theres an Autosave file present
    ...
    def newGame(self):
        ...
        self.scoreBtn = ttk.Button(
            self.diceframe, text="Nästa kategori", command=self.newGame
        )
        ...
        ttk.Label(self.scoreframe, textvariable=self.kastnr).grid(
            row=18, column=1, pady=10, sticky=E
        )
        ...
        ttk.Label(self.scoreframe, textvariable=self.catnr).grid(
            row=19, column=1, sticky=E
        )
        ...
        self.rollBtn = ttk.Button(
            self.diceframe, text="Kasta om", command=self.rollDice
        )
    ...
    def rollDice(self):
        ...
        for n in range(5):
            ...
            check = ttk.Checkbutton(
                self.diceframe, text=f"Lås tärning {n+1}", variable=var
            )
    ...
    def scoreDice(self):
        ttk.Label(self.scoreframe, text="Kategori", width=15).grid(row=0, column=0)
        self.scorelist = [
        ...
        for val, scoreObj in enumerate(self.scorelist):
            ...
            r = Radiobutton(
                self.scoreframe,
                text=scoreObj[0],
                command=self.enableScore,
                variable=self.sel_score,
                value=val,
                width=15,
                padx=5,
                anchor=E,
            ...
            ttk.Label(self.scoreframe, text=scoretext, width=3, relief="raised").grid(
                padx=2, row=rownr, column=1
            )
            ttk.Label(
                self.scoreframe, text=self.scorecard[val], width=3, relief="raised"
            ).grid(padx=2, row=rownr, column=2)
            ...
            if val == 6:  # Insert bonus rows
                ...
                ttk.Label(
                    self.scoreframe, text=self.scorecard[16], width=3, relief="raised"
                ).grid(padx=2, row=7, column=2)
        ...
        ttk.Label(self.scoreframe, text=scorecard_sum, width=3, relief="raised").grid(
            padx=2, row=17, column=2
        )
    ...
    def gameWon(self):
        ...
        ttk.Label(
            self.diceframe, text=f"Grattis!\nDu vann spelet med {score} poäng"
        ).grid(row=3, column=0)
        ttk.Button(self.diceframe, text="Nytt spel", command=self.playAgain).grid(
            row=4, column=1, pady=10
        )
        resetSave()
        ttk.Button(self.diceframe, text="Avsluta", command=quit).grid(
